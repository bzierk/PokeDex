...
"weight": 69
  ...
  const [pokeImage, setPokeImage] = useState({"img_url":"http://cdn2.bulbagarden.net/upload/thumb/2/21/001Bulbasaur.png/250px-001Bulbasaur.png"})
...
const fetchImage = async(id) => {
  const res = await fetch(`https://cs361service.herokuapp.com/pokemon/${id}`)
  const data = res.json()
...
const subSearch = async (search) => {
  ...
  const flavorText = await fetchFlavorText(fixedSearch)
  // const pokeImage = await fetchImage(pokeData.name)
  ...
  return (
    <div className="pokedex">
      <LeftPanel onSearch={subSearch} pokeData={pokeData} pokeImage={pokeImage}/>
      <RightPanel pokeData={pokeData} flavorText={flavorText} />
    </div>
  );
}
